...
func (ss NameserverCheckList) All(fn func(value *NameserverCheck) bool) bool {
...
func (ss NameserverCheckList) Any(fn func(value *NameserverCheck) bool) bool {
	for _, value := range ss {
		if fn(value) {
			return true
		}
...
func (ss NameserverCheckList) Append(elements ...*NameserverCheck) NameserverCheckList {
	// Copy ss, to make sure no memory is overlapping between input and
	// output. See issue #97.
	result := append(NameserverCheckList{}, ss...)
...
func (ss NameserverCheckList) Bottom(n int) (top NameserverCheckList) {
...
func (ss NameserverCheckList) Contains(lookingFor *NameserverCheck) bool {
	for _, s := range ss {
		if lookingFor == s {
			return true
		}
...
func (ss NameserverCheckList) Diff(against NameserverCheckList) (added, removed NameserverCheckList) {
	...
	diffOneWay := func(ss1, ss2raw NameserverCheckList) (result NameserverCheckList) {
		ss2 := make(NameserverCheckList, len(ss2raw))
		copy(ss2, ss2raw)
...
// if n < 0 it'll return empty slice.
func (ss NameserverCheckList) DropTop(n int) (drop NameserverCheckList) {
	if n < 0 || n >= len(ss) {
		return
	}
	...
	drop = make([]*NameserverCheck, len(ss)-n)
	copy(drop, ss[n:])
...
//   })
//
func (ss NameserverCheckList) Each(fn func(*NameserverCheck)) NameserverCheckList {
	for _, s := range ss {
		fn(s)
	}
...
func (ss NameserverCheckList) Equals(rhs NameserverCheckList) bool {
	if len(ss) != len(rhs) {
		return false
	}
...
//
// It is acceptable to provide zero arguments.
func (ss NameserverCheckList) Extend(slices ...NameserverCheckList) (ss2 NameserverCheckList) {
	ss2 = ss
...
func (ss NameserverCheckList) Filter(condition func(*NameserverCheck) bool) (ss2 NameserverCheckList) {
	for _, s := range ss {
		if condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss NameserverCheckList) FilterNot(condition func(*NameserverCheck) bool) (ss2 NameserverCheckList) {
	for _, s := range ss {
		if !condition(s) {
			ss2 = append(ss2, s)
		}
...
func (ss NameserverCheckList) FindFirstUsing(fn func(value *NameserverCheck) bool) int {
...
func (ss NameserverCheckList) First() *NameserverCheck {
	return ss.FirstOr(nil)
}

// FirstOr returns the first element or a default value if there are no
// elements.
func (ss NameserverCheckList) FirstOr(defaultValue *NameserverCheck) *NameserverCheck {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss NameserverCheckList) Float64s() pie.Float64s {
	l := len(ss)
...
func (ss NameserverCheckList) Insert(index int, values ...*NameserverCheck) NameserverCheckList {
	if index >= ss.Len() {
		return NameserverCheckList.Extend(ss, NameserverCheckList(values))
	}

	return NameserverCheckList.Extend(ss[:index], NameserverCheckList(values), ss[index:])
}

// Ints transforms each element to an integer.
func (ss NameserverCheckList) Ints() pie.Ints {
	l := len(ss)
...
func (ss NameserverCheckList) Join(glue string) (s string) {
	var slice interface{} = []*NameserverCheck(ss)
...
func (ss NameserverCheckList) JSONBytes() []byte {
	if ss == nil {
		return []byte("[]")
	}
...
func (ss NameserverCheckList) JSONBytesIndent(prefix, indent string) []byte {
	if ss == nil {
		return []byte("[]")
	}
...
// One important thing to note is that it will treat a nil slice as an empty
// slice to ensure that the JSON value return is always an array.
func (ss NameserverCheckList) JSONString() string {
	if ss == nil {
		return "[]"
	}
...
// slice to ensure that the JSON value return is always an array. See
// json.MarshalIndent for details.
func (ss NameserverCheckList) JSONStringIndent(prefix, indent string) string {
	if ss == nil {
		return "[]"
	}
...
func (ss NameserverCheckList) Last() *NameserverCheck {
	return ss.LastOr(nil)
}

// LastOr returns the last element or a default value if there are no elements.
func (ss NameserverCheckList) LastOr(defaultValue *NameserverCheck) *NameserverCheck {
	if len(ss) == 0 {
		return defaultValue
	}
...
func (ss NameserverCheckList) Len() int {
	return len(ss)
}
...
func (ss NameserverCheckList) Map(fn func(*NameserverCheck) *NameserverCheck) (ss2 NameserverCheckList) {
	if ss == nil {
		return nil
	}

	ss2 = make([]*NameserverCheck, len(ss))
	for i, s := range ss {
		ss2[i] = fn(s)
	}
...
func (ss NameserverCheckList) Mode() NameserverCheckList {
	if len(ss) == 0 {
		return nil
	}
	values := make(map[*NameserverCheck]int)
	for _, s := range ss {
		values[s]++
	}
	...
	var maxValues NameserverCheckList
...
//   }
func (ss *NameserverCheckList) Pop() (popped **NameserverCheck) {
...
func (ss NameserverCheckList) Random(source rand.Source) *NameserverCheck {
	n := len(ss)
...
//   ss.Sort().Reverse()
//
func (ss NameserverCheckList) Reverse() NameserverCheckList {
	...
	sorted := make([]*NameserverCheck, len(ss))
	for i := 0; i < len(ss); i++ {
...
func (ss NameserverCheckList) Send(ctx context.Context, ch chan<- *NameserverCheck) NameserverCheckList {
	for i, s := range ss {
		select {
		case <-ctx.Done():
			return ss[:i]
...
func (ss NameserverCheckList) SequenceUsing(creator func(int) *NameserverCheck, params ...int) NameserverCheckList {
	var seq = func(min, max, step int) (seq NameserverCheckList) {
		lenght := int(util.Round(float64(max-min) / float64(step)))
		if lenght < 1 {
			return
		}

		seq = make(NameserverCheckList, lenght)
...
func (ss NameserverCheckList) Shift() (*NameserverCheck, NameserverCheckList) {
	return ss.First(), ss.DropTop(1)
}

// Shuffle returns shuffled slice by your rand.Source
func (ss NameserverCheckList) Shuffle(source rand.Source) NameserverCheckList {
	n := len(ss)
	...
	shuffled := make([]*NameserverCheck, n)
	copy(shuffled, ss)
...
func (ss NameserverCheckList) SortStableUsing(less func(a, b *NameserverCheck) bool) NameserverCheckList {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
	...
	sorted := make(NameserverCheckList, len(ss))
	copy(sorted, ss)
...
// slice returned will be reallocated as to not modify the input slice.
func (ss NameserverCheckList) SortUsing(less func(a, b *NameserverCheck) bool) NameserverCheckList {
	// Avoid the allocation. If there is one element or less it is already
	// sorted.
	if len(ss) < 2 {
		return ss
...
//   fmt.Sprintf("%v")
//
func (ss NameserverCheckList) Strings() pie.Strings {
	l := len(ss)
...
func (ss NameserverCheckList) SubSlice(start int, end int) (subSlice NameserverCheckList) {
	if start < 0 || end < 0 {
		return
	}
	...
	if start < length {
		if end <= length {
			...
			zeroArray := make([]*NameserverCheck, end-length)
			subSlice = ss[start:length].Append(zeroArray[:]...)
		}
	} else {
		zeroArray := make([]*NameserverCheck, end-start)
		subSlice = zeroArray[:]
	}
...
// if the slice has less elements then n that'll return all elements
// if n < 0 it'll return empty slice.
func (ss NameserverCheckList) Top(n int) (top NameserverCheckList) {
...
func (ss NameserverCheckList) StringsUsing(transform func(*NameserverCheck) string) pie.Strings {
	l := len(ss)
...
func (ss NameserverCheckList) Unshift(elements ...*NameserverCheck) (unshift NameserverCheckList) {
	unshift = append(NameserverCheckList{}, elements...)
